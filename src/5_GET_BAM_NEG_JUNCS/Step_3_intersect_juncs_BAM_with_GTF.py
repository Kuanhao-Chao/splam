...
def main():
    ...
    print("bam_juncs: ", bam_juncs)
    print("ref_juncs: ", ref_juncs)
    ...
    left_df = pd.merge(bam_juncs, ref_juncs, how ='left', on =[0, 1, 2, 5])
    print("left_df: ", left_df)

    noref_df= left_df[~left_df["3_y"].notna() & ~left_df["4_y"].notna()]
    print(len(noref_df))

    out_df = noref_df[[0, 1, 2, "3_x", "4_x", 5]]
    out_df = out_df.rename(columns={0:"chr",1:"start", 2:"end", "3_x":"junc", "4_x":"score", 5:"strand"})
    print("out_df: ", out_df)
