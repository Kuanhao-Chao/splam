...
class SPLAMOutputTarget:
    def __init__(self):
        pass

    def __call__(self, model_output, model_label):

        SEQ_WEIGHT = 4
        # This is cross entropy loss
        print("model_output: ", model_output.shape)
        print("model_label : ", model_label.shape)
        return - torch.mean(model_label[0, :]*torch.log(model_output[0, :]+1e-10)
                            + model_label[1, :]*torch.log(model_output[1, :]+1e-10)
                            + model_label[2, :]*torch.log(model_output[2, :]+1e-10))
        # return torch.softmax(model_output, dim=-1)
