...
class myDataset(Dataset):
    def __init__(self, data_dir, segment_len=10):
        ...
        mapping_path = Path(data_dir) / "tranx_feature/processed_gene.txt"
        # mapping = json.load(mapping_path.open())
        f_mapping_path = open(mapping_path, 'r')
        geneIDs = f_mapping_path.read().splitlines()
 
        # Load the mapping from speaker neme to their corresponding id. 
        features_path = Path(data_dir) / "geneid_2_features.txt"
        features = json.load(features_path.open())
 
        # Get the total number of speaker.
        self.data = []
        counter = 0
        for geneID in geneIDs:
            # print("utterances['feature_path']: ", utterances["feature_path"])
            # print(features[geneID])
            # print(features[geneID]["feature"])
            # print(features[geneID]["label"])
            self.data.append([features[geneID]["feature"], features[geneID]["label"]])
            counter += 1
    ...
    def __getitem__(self, index):
        ...
        feature = torch.load(self.data[index][0])
        label = torch.load(self.data[index][1])
        ...
        if len(feature) > self.segment_len:
            # Randomly get the starting point of the segment.
            start = random.randint(0, len(feature) - self.segment_len)
            # Get a segment with "segment_len" frames.
            feature = torch.FloatTensor(feature[start:start+self.segment_len])
            label = torch.FloatTensor(label[start:start+self.segment_len])
        else:
            feature = torch.FloatTensor(feature)
        # Turn the speaker id into long for computing loss later.
...
def collate_batch(batch):
    ...
    feature = pad_sequence(feature, batch_first=True, padding_value=-20)    # pad -20 which is very small value.
    # mel: (batch size, length, 24)
    return feature, label
...
class Classifier(nn.Module):
    def __init__(self, config, seq_len, d_model=80, dropout=0.1):
        super().__init__()
        # Project the dimension of features from that of input into d_model.
        self.prenet = nn.Linear(24, d_model)
        # TODO:
...
def model_fn(batch, model, criterion, device):
    ...
    mels, labels = batch # size [batch size, length, 24] , [batch size, length]
