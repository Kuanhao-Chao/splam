...
model = torch.load("../src/MODEL/SpliceAI_6_RB_p_n_nn_n1_TB_all_samples_thr_100_splitByChrom_L64_C16_L800_v21/SpliceNN_19.pt")
...
MODEL_OUTPUT_BASE = "./TEST/SpliceAI_6_RB_p_n_nn_n1_TB_all_samples_thr_100_splitByChrom_L64_C16_L800_v20/"
...
def test_one_epoch(epoch_idx, test_loader):
    ...
    # All_Acceptor_YL = np.array([])
    # All_Acceptor_YP = np.array([])
    # All_Donor_YL = np.array([])
    # All_Donor_YP = np.array([])
    ...
    for batch_idx, data in enumerate(test_loader):
        ...
        donor_label = labels[:, 2, 200].to("cpu").detach().numpy()
        donor_predict = yp[:, 2, 200].to("cpu").detach().numpy()

        acceptor_label = labels[:, 1, 600].to("cpu").detach().numpy()
        acceptor_prediction = yp[:, 1, 600].to("cpu").detach().numpy()

        print("donor_label: ", donor_label)
        print("donor_predict: ", donor_predict)
        print("acceptor_label: ", acceptor_label)
        print("acceptor_prediction: ", acceptor_prediction)

        # np.append(All_Donor_YL, donor_label)
        # np.append(All_Donor_YP, donor_predict)
        # np.append(All_Acceptor_YL, acceptor_label)
        # np.append(All_Acceptor_YP, acceptor_prediction)
        ...
        All_Acceptor_YL.extend(acceptor_label)
        All_Acceptor_YP.extend(acceptor_prediction)
        All_Donor_YL.extend(donor_label)
        All_Donor_YP.extend(donor_predict)

        print("All_Donor_YL: ", len(All_Donor_YL))
        print("All_Donor_YP: ", len(All_Donor_YP))
    ...
    with open("./INPUT/splam.pkl", 'wb') as f: 
        # d_pred_prob = []
        # d_label_prob = [1]*POS_NUM + [0]*POS_NUM
        # a_pred_prob = []
        # a_label_prob = [1]*POS_NUM + [0]*POS_NUM

        pickle.dump(All_Donor_YP, f)
        pickle.dump(All_Donor_YL, f)
        pickle.dump(All_Acceptor_YP, f)
        pickle.dump(All_Acceptor_YL, f)
