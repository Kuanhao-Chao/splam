...
def main():
    ...
    for threshold in THRESHOLDS:
        ...
        os.makedirs(figure_root+"tsv_"+str(threshold)+"/", exist_ok=True)
        ...
        with open("./spliceai_result/spliceai.noN.merged.pkl",'rb') as f:
            ...
            spliceai_noN_junc_name = pickle.load(f)
        ...
        spliceai_noN_junc_name = np.array(spliceai_noN_junc_name)
        ...
        print("spliceai_noN_junc_name: ", spliceai_noN_junc_name)
        ...
        print("spliceai_noN_junc_name: ", len(spliceai_noN_junc_name))
        ...
        # print(spliceai_noN_d_pred_prob >= threshold)
        spliceai_TP_idices = (spliceai_noN_d_pred_prob >= threshold) & (spliceai_noN_a_pred_prob >= threshold) & (spliceai_noN_d_label_prob == 1)
        spliceai_FN_idices = ((spliceai_noN_d_pred_prob < threshold) | (spliceai_noN_a_pred_prob < threshold)) & (spliceai_noN_d_label_prob == 1)
        spliceai_FP_idices = (spliceai_noN_d_pred_prob >= threshold) & (spliceai_noN_a_pred_prob >= threshold) & (spliceai_noN_d_label_prob == 0)
        spliceai_TN_idices = ((spliceai_noN_d_pred_prob < threshold) | (spliceai_noN_a_pred_prob < threshold)) & (spliceai_noN_d_label_prob == 0)

        print("spliceai_TP_idices : ", len(spliceai_noN_junc_name[spliceai_TP_idices]))
        print("spliceai_FP_idices : ", len(spliceai_noN_junc_name[spliceai_FP_idices]))
        print("spliceai_FN_idices : ", len(spliceai_noN_junc_name[spliceai_FN_idices]))
        print("spliceai_TN_idices : ", len(spliceai_noN_junc_name[spliceai_TN_idices]))
        ...
        for TYPE in ["TP_TP", "TP_FN", "FN_TP", "FN_FN", "TN_TN", "TN_FP", "FP_TN", "FP_FP"]:
            ...
            fw = open(figure_root+"tsv_"+str(threshold)+"/"+TYPE+"_junc_"+str(threshold)+".tsv", "w")
        ...
        for TOOL in ["spliceai", "splam"]:
            if TOOL == "spliceai":
                d_score = spliceai_noN_d_pred_prob
                a_score = spliceai_noN_a_pred_prob
            elif TOOL == "splam":
            ...
            plt.savefig(figure_root + TOOL + "_" + str(threshold) + ".png", dpi=300, bbox_extra_artists=(lgd,), bbox_inches='tight')
        ...
        # plot_scatter_plot(spliceai_noN_d_label_prob[spliceai_FP_idices], spliceai_noN_d_pred_prob, spliceai_noN_a_label_prob, spliceai_noN_a_pred_prob, figure_root+"spliceai_noN.png")


        # print(spliceai_noN_d_pred_prob[spliceai_noN_d_pred_prob > threshold])

        # spliceai_noN_d_label_prob[spliceai_noN_d_label_prob > threshold]
        # spliceai_noN_a_pred_prob[spliceai_noN_a_pred_prob > threshold]
        # spliceai_noN_a_label_prob[spliceai_noN_a_label_prob > threshold]
        # spliceai_noN_junc_name


        # for x, y in zip(spliceai_noN_junc_name, splam_noS_junc_name):
