...
def plot_density_scatter_plot(d_pred, a_pred):
    plt.rcParams['font.size'] = 8
    # plt.subplots_adjust(wspace=0.4, hspace=-0.5)

    selected_indices_combined = np.vstack([d_pred, a_pred])

    density_combined = gaussian_kde(selected_indices_combined)

    density_values = density_combined(selected_indices_combined)

    vmin = 0 #min(density_1_values.min(), density_2_values.min())-100
    vmax = density_values.max()
    print("vmax: ", vmax)
    norm = plt.Normalize(vmin=vmin, vmax=vmax)
    cmap = plt.get_cmap('plasma')  # You can choose a different colormap if desired

    plt.scatter(d_pred, a_pred, s=0.7, c=density_values, cmap=cmap, alpha=1.0, norm=norm)
...
def main():
    ...
    for threshold in THRESHOLDS:
        ...
        for SPLAM_VERSION in ["SPLAM_v11"]:#, "SPLAM_v12"]:
            for TYPE in ["FULL"]:#, "pos_MANE", "pos_ALTS", "BOTH"]:
                #####################################
                # Creating directories for visualization.
                #####################################
                figure_root = "./IMG/"+SPLAM_VERSION+"/scores/"
                os.makedirs(figure_root, exist_ok=True)

                with open("../../src_tools_evaluation/splam_result/"+SPLAM_VERSION+"/splam.da.noshuffle.merged."+TYPE+".pkl",'rb') as f:
                    ...
                    plot_density_scatter_plot(splam_d_pred, splam_a_pred)
                    # Save the plot
                    plt.savefig(figure_root + "splam."+TYPE + ".png", dpi=600, bbox_inches='tight')
                ...
                for TARGET in ["noN", "N"]:
                    ...
                    with open("../../src_tools_evaluation/spliceai_result_AVERAGE/spliceai.da."+TARGET+".merged."+TYPE+".pkl", "rb") as fr:
                    ...
                    plot_density_scatter_plot(spliceai_d_pred, spliceai_a_pred)

                    # Save the plot
                    plt.savefig(target_figure_root + "spliceai."+TYPE + ".png", dpi=600, bbox_inches='tight')
