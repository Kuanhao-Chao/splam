...
os.makedirs("novel_missed", exist_ok=True)
    ...
    for level in ["exons", "introns", "loci"]:
        for library in ["polyA", "ribozero"]:
            for annotation in annotations:
                ...
                plt.bar(rowname, difference_target_ls, color = colors)
                plt.xlabel('Samples')
                plt.ylabel('Number of '+ target + ' ' + level + " (%)")
                plt.title('Precision vs. Recall (' + annotation + ')')

                plt.tight_layout()
                plt.savefig("novel_missed/" + target+ "_" + level + "_" + library + "_" + annotation + ".png", dpi=300)
